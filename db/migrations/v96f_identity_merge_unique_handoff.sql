-- CYA Hub v96f — safe unique identity handoff during merge.
begin;
...
begin
  ...
  if p_source_person_id is null or p_target_person_id is null or p_source_person_id=p_target_person_id then raise exception 'Selecciona dos fichas distintas.' using errcode='22023'; end if;
  ...
  if nullif(btrim(coalesce(p_match_email,'')),'') is not null or nullif(btrim(coalesce(p_match_phone,'')),'') is not null then
    ...
    if v_verified_match is distinct from p_target_person_id then raise exception 'La identidad indicada ya no coincide de forma única con la ficha de destino.' using errcode='23505'; end if;
  end if;
  ...
  loop
    execute format('select exists(select 1 from %I.%I where %I=$1)',v_fk.table_schema,v_fk.table_name,v_fk.column_name) into v_has_rows using p_source_person_id;
    if v_has_rows then raise exception 'Esta ficha registrada ya tiene actividad asociada y no puede fusionarse automáticamente sin revisar su historial.' using errcode='55000'; end if;
  end loop;

  -- Release unique identity values on the duplicate first; the transaction guarantees atomicity.
