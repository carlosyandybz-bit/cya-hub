-- P30C3 — Métricas de alumnado y negocio.
...
set search_path=''
as $$
declare v_student bigint; v_country text; v_value numeric;
begin
  ...
  if p_key='students_active' then
    select count(*) into v_value from public.student_profiles sp join public.people p on p.id=sp.person_id
    where sp.active and (v_country is null or p.country_code=v_country);
  elsif p_key='new_students' then
    select count(*) into v_value from public.student_profiles sp join public.people p on p.id=sp.person_id
    where sp.student_since>=p_from::date and sp.student_since<p_to::date+1 and (v_country is null or p.country_code=v_country);
    ...
    from public.credit_grants cg where cg.purchased_at>=p_from and cg.purchased_at<p_to and cg.payment_status='paid'
  ...
  else raise exception 'Métrica de alumnado/negocio no soportada.' using errcode='22023'; end if;
  return v_value;
end;
$$;
