...
drop policy if exists cya_icons_calendar_admin_insert on storage.objects;
drop policy if exists cya_icons_calendar_admin_update on storage.objects;
drop policy if exists cya_icons_calendar_admin_delete on storage.objects;

create policy cya_icons_calendar_admin_insert on storage.objects
for insert to authenticated with check (
  bucket_id = 'cya-icons' and (storage.foldername(name))[1] = 'calendar' and (select private.is_admin())
);
create policy cya_icons_calendar_admin_update on storage.objects
for update to authenticated using (
  bucket_id = 'cya-icons' and (storage.foldername(name))[1] = 'calendar' and (select private.is_admin())
) with check (
  bucket_id = 'cya-icons' and (storage.foldername(name))[1] = 'calendar' and (select private.is_admin())
);
create policy cya_icons_calendar_admin_delete on storage.objects
