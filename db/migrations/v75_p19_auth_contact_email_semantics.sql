-- P19 follow-up — people.email is canonical contact identity data, not necessarily the Auth login email.
-- Keep phone/email deduplication strict for unlinked people, while allowing an already-linked person
-- to retain a different contact email when auth.users is the unambiguous access identity.
...
begin
  ...
  -- First preserve P19's canonical contact identity matching.
  select private.match_person_identity(v_email,v_phone,null) into v_person_id;

  -- A registered person's contact email may differ from their login. If contact matching did not
  -- find the person, recover the existing canonical person through the Auth login identity.
  if v_person_id is null then
    select u.id
    into v_existing_auth_user_id
    from auth.users u
    where private.normalize_person_email(u.email)=v_email
    limit 1;
    ...
    if v_existing_auth_user_id is not null then
      ...
      where p.active and p.auth_user_id=v_existing_auth_user_id
      limit 1;
    end if;
  end if;
  ...
  if v_person_id is not null then
    ...
    if v_person.auth_user_id is not null then
      select private.normalize_person_email(u.email)
      into v_linked_auth_email
      from auth.users u
      where u.id=v_person.auth_user_id;

      if v_linked_auth_email is null or v_linked_auth_email is distinct from v_email then
        raise exception 'La ficha encontrada ya está vinculada a otra cuenta de acceso. Revísala antes de continuar.' using errcode='23505';
      end if;
    elsif private.normalize_person_email(v_person.email) is not null
          and private.normalize_person_email(v_person.email) is distinct from v_email then
      -- With no Auth link yet, a phone-only match against another stored contact email remains
      -- intentionally conservative: administration must review the identity before linking access.
  ...
  return jsonb_build_object(
    ...
    'auth_user_id',coalesce(v_person.auth_user_id,v_existing_auth_user_id),
...
begin
  ...
  -- When contact email differs from login and no phone was supplied, recover the already-linked
  -- canonical person by auth_user_id instead of trying to create a duplicate person.
  if v_person_id is null then
    select p.id
    into v_person_id
    from public.people p
    where p.active and p.auth_user_id=p_auth_user_id
    limit 1;
  end if;
  ...
  if v_person_id is not null then
    ...
    if v_person.auth_user_id is null
       and private.normalize_person_email(v_person.email) is not null
       and private.normalize_person_email(v_person.email) is distinct from v_email then
