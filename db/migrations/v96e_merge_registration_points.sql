...
begin
  ...
  -- If explicit identity evidence exists, verify it. For admin-selected merges from the provisional side,
  -- the target data itself is used as evidence and may be partial.
  ...
  -- Automatic registration can create point-ledger rows immediately. Those are safe to preserve and reassign.
  -- Any other source-side business/history relation still blocks the automatic merge.
  for v_fk in
