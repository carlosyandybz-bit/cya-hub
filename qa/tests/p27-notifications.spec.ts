...
test("P27 administration shows the real automatic notification engine",async({page})=>{
  ...
  await expect(page.getByRole("heading",{name:"Estado real de conexiones"})).toBeVisible();
...
test("P27 notification rules expose quiet hours without claiming external delivery",async({page})=>{
  ...
  await expect(page.getByText(/las horas silenciosas se conservan para canales externos cuando estén disponibles/i)).toBeVisible();
});
