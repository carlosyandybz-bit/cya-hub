...
create_start = app.index("  async function createCorrection()")
create_end = app.index("  async function updateCorrection(", create_start)
...
"""
app = app[:create_start] + replacement + app[create_end:]

# 5. Replace search panel with contextual results and type-specific quick actions.
search_token = '<section className="live-unified-search card">'
start_token = app.index(search_token)
start = app.rfind('\n', 0, start_token) + 1
end_token = app.index('<nav className="live-work-tabs">', start_token)
end = app.rfind('\n', 0, end_token) + 1
