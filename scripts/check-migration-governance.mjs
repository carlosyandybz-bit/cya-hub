...
export function validateProvenanceRecord(record, provenanceSchema, options = {}) {
  ...
  return [
    ...validateRecordSchema(
      ...
      options.compiledValidator ?? null,
    ),
...
export function validateProvenanceRegistry(registry, schema, options = {}) {
  const errors = [];
  if (registry?.schema_version !== CORE01_SCHEMA_VERSION) {
    errors.push(`${PROVENANCE_REGISTRY_PATH}: schema_version debe ser ${CORE01_SCHEMA_VERSION}.`);
  }
  if (registry?.record_schema !== PROVENANCE_SCHEMA_PATH) {
    errors.push(`${PROVENANCE_REGISTRY_PATH}: record_schema debe apuntar a ${PROVENANCE_SCHEMA_PATH}.`);
  }
  if (schema?.["x-cya-schema-version"] !== CORE01_SCHEMA_VERSION) {
    errors.push(`${PROVENANCE_SCHEMA_PATH}: x-cya-schema-version debe ser ${CORE01_SCHEMA_VERSION}.`);
  }
  if (schema?.title !== "CORE-01 post-contract migration provenance") {
    errors.push(`${PROVENANCE_SCHEMA_PATH}: title inesperado; contrato machine-readable no reconocido.`);
  }
  ...
  if (!compiledValidator) {
    try {
      compiledValidator = compileProvenanceSchema(schema, options.rootDir ?? process.cwd());
    } catch (error) {
      errors.push(`${PROVENANCE_SCHEMA_PATH}: no pudo compilarse con Ajv Draft 2020-12 (${error.message}).`);
      return errors;
    }
  }
  ...
  if (duplicates.length) {
    errors.push(`${PROVENANCE_REGISTRY_PATH}: rutas duplicadas: ${[...new Set(duplicates)].join(", ")}`);
  }

  for (const record of registry.migrations) {
    errors.push(...validateProvenanceRecord(record, schema, { ...options, compiledValidator }));
  }
  return errors;
}
...
export function validateChanges(changes, manifest, provenanceRegistry, provenanceSchema, options = {}) {
    ...
    for (const path of change.paths) {
      ...
      if (isUnder(path, "supabase/migrations") && path.endsWith(".sql")) {
        if (change.status.startsWith("A")) {
          ...
          if (!artifact) {
            errors.push(`${path}: falta registrar la nueva migración en ${PROVENANCE_REGISTRY_PATH}.`);
            continue;
          }
          ...
          if (artifact.applied_state !== "PREPARADA_NO_APLICADA") {
            errors.push(`${path}: una migración añadida en este PR debe comenzar PREPARADA_NO_APLICADA; APLICADA solo puede promoverse tras Release/post-apply.`);
          }
  ...
  if (new Set(addedTimestamps).size !== addedTimestamps.length) {
    errors.push("Las nuevas migraciones reutilizan un timestamp UTC de 14 dígitos.");
  }

  const historicalTimestamps = artifacts
    .filter((artifact) => artifact.path.startsWith("supabase/migrations/") && !addedCanonical.includes(artifact.path))
    .map((artifact) => canonicalTimestamp(artifact.path))
    .filter(Boolean);
  ...
  const floor = [maxHistorical, ledgerFloor]
    .filter((value) => /^\d{14}$/.test(value ?? ""))
    .sort()
    .at(-1) ?? null;
  if (floor) {
    for (const timestamp of addedTimestamps) {
      if (timestamp <= floor) {
        errors.push(`Timestamp ${timestamp}: debe ser posterior al máximo histórico/ledger ${floor}; no se admiten migraciones nuevas retrodatadas.`);
      }
    }
  }
...
export function validateInventory(actualByRoute, manifest, provenanceRegistry, provenanceSchema, options = {}) {
  ...
  const duplicates = paths.filter((path, index) => paths.indexOf(path) !== index);
  if (duplicates.length) {
    errors.push(`Manifiesto/registro con rutas duplicadas: ${[...new Set(duplicates)].join(", ")}`);
  }
  ...
  for (const route of ROUTES) {
    ...
    for (const path of actual) {
      if (!declaredSet.has(path)) {
        errors.push(`${path}: SQL presente en ${route} pero ausente del inventario combinado histórico/provenance.`);
      }
    }
    for (const path of declared) {
      if (!actualSet.has(path)) {
        errors.push(`${path}: declarado en inventario/provenance pero ausente del repositorio.`);
      }
    }
  }
  ...
  if (totals) {
    ...
    const states = Object.fromEntries(
      ["APLICADA", "PREPARADA_NO_APLICADA", "DESCONOCIDA"]
        .map((state) => [state, historical.filter((artifact) => artifact.applied_state === state).length]),
    );
    if (totals.artifacts !== historical.length) {
      errors.push(`inventory_totals.artifacts=${totals.artifacts} no coincide con snapshot histórico ${historical.length}.`);
    }
    for (const [state, count] of Object.entries(states)) {
      if (totals[state] !== count) {
        errors.push(`inventory_totals.${state}=${totals[state]} no coincide con snapshot histórico ${count}.`);
      }
    }
  }
...
export async function validateContextAgainstGitHub(records, changes, context, options = {}) {
  const addedPaths = new Set();
  for (const change of changes) {
    for (const path of change.paths) {
      if (change.status.startsWith("A") && isUnder(path, "supabase/migrations") && path.endsWith(".sql")) {
        addedPaths.add(path);
      }
    }
  }
  ...
  for (const record of records) {
    errors.push(...await validateContextualProvenance(record, context, {
      ...options,
      isNewMigration: addedPaths.has(record.path),
    }));
  }
  return [...new Set(errors)];
}
...
async function main() {
  ...
  try {
    compiledValidator = compileProvenanceSchema(provenanceSchema, rootDir);
  } catch (error) {
    console.error(`CORE-01 MIGRATION GOVERNANCE: FAIL\n- JSON Schema Draft 2020-12 no compilable con Ajv: ${error.message}`);
    process.exit(1);
  }

  const validationOptions = { rootDir, compiledValidator };
  const actualByRoute = Object.fromEntries(ROUTES.map((route) => [route, walkSql(rootDir, route)]));
  let errors = [
    ...validateInventory(actualByRoute, manifest, provenanceRegistry, provenanceSchema, validationOptions),
  ...
  if (base) {
    ...
    errors.push(...validateChanges(changes, manifest, provenanceRegistry, provenanceSchema, validationOptions));
  }

  if (process.env.GITHUB_ACTIONS === "true") {
    try {
      const context = loadGitHubAuthoringContext();
      errors.push(...await validateContextAgainstGitHub(
        registryMigrations(provenanceRegistry),
        changes,
        context,
        { cwd: rootDir },
      ));
    } catch (error) {
      errors.push(`Contexto Git/GitHub no corroborable: ${error.message}`);
    }
  }
  ...
  console.log(`JSON Schema: Draft 2020-12 / Ajv / schema v${CORE01_SCHEMA_VERSION}`);
  ...
  if (process.env.GITHUB_ACTIONS === "true") {
    console.log("Authoring provenance: corroborated against Git/GitHub context; APPLIED requires successful post-apply verification run.");
  }
}
