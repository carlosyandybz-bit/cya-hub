...
catalog = Path("app/statistics-catalog.ts")
...
text = one(
    ...
    "statistics block type",
)
text = one(text, '  bz: "BZ Points",\n  marketing: "Marketing",', '  bz: "BZ Points",\n  feedback: "Feedback Online",\n  marketing: "Marketing",', "statistics block label")
anchor = '  { key:"bz_redemptions", block:"bz", label:"Recompensas canjeadas", format:"number", filters:["student"], description:"Cupones o descuentos creados mediante canje durante el periodo." },\n\n'
addition = anchor + '''  { key:"feedback_submitted", block:"feedback", label:"Feedback enviados", format:"number", filters:["student","style"], description:"Solicitudes de Feedback Online enviadas durante el periodo." },
...
'''
text = one(text, anchor, addition, "statistics feedback catalog")
catalog.write_text(text)

engine = Path("app/statistics-engine.ts")
text = engine.read_text()
anchor = '  throw new Error("Métrica BZ no soportada.");\n}\n\nexport async function calculateStatistic'
helper = '''  throw new Error("Métrica BZ no soportada.");
}
...
export async function calculateStatistic'''
text = one(text, anchor, helper, "statistics feedback helper")
text = one(
    text,
    ...
    "statistics feedback router",
)
engine.write_text(text)

transfer = Path("app/admin-data-transfer.tsx")
text = transfer.read_text()
text = one(text, '  ["bz", "BZ Points y recompensas"],\n  ["settings", "Configuración"],', '  ["bz", "BZ Points y recompensas"],\n  ["feedback", "Feedback Online"],\n  ["settings", "Configuración"],', "export feedback domain")
text = one(text, '  ["bz", "BZ Points y recompensas"],\n];\n', '  ["bz", "BZ Points y recompensas"],\n  ["feedback", "Feedback Online"],\n];\n', "import feedback domain")
text = one(text, '    setBusy("preview-import");\n    try {\n      const parsed = await parseImportFile(importFile);', '    setBusy("preview-import");\n    try {\n      const parsed = await parseImportFile(importFile);\n      if (parsed.kind === "rows" && importDomain === "feedback") throw new Error("Feedback Online se importa desde una copia JSON exportada por CYA Hub, no desde CSV/Excel plano.");', "feedback raw import guard")
transfer.write_text(text)

reset = Path("app/admin-data-reset.tsx")
text = reset.read_text()
text = one(text, 'description: "Perfiles de alumno, clases, bonos, evaluaciones, medidas, asignaciones, incidencias e historial BZ. Los contactos CRM se conservan.",', 'description: "Perfiles de alumno, clases, bonos, evaluaciones, medidas, asignaciones, incidencias, historial BZ y actividad de Feedback Online. Los contactos CRM se conservan.",', "reset students copy")
text = one(text, 'description: "Correcciones, explicaciones, ejercicios, secuencias, relaciones, multimedia y sus asignaciones/medidas.",', 'description: "Correcciones, explicaciones, ejercicios, secuencias, relaciones, multimedia, asignaciones/medidas y vínculos de Feedback Online con ese contenido.",', "reset teaching copy")
text = one(text, 'description: "Borra personas no vinculadas al equipo, alumnado, clases, bonos, evaluaciones, CRM, marketing, misiones, agenda, avisos, formularios enviados e historial BZ. Conserva la biblioteca de enseñanza y la configuración de BZ.",', 'description: "Borra personas no vinculadas al equipo, alumnado, clases, bonos, evaluaciones, CRM, marketing, misiones, agenda, avisos, formularios enviados, historial BZ y actividad de Feedback Online. Conserva la biblioteca de enseñanza y la configuración de BZ/Feedback.",', "reset operational copy")
text = one(text, 'description: "Deja la aplicación vacía de datos de negocio y contenido creado: alumnado, personas de prueba, enseñanza, clases, bonos, evaluaciones, medidas, CRM, marketing, estadísticas, misiones, agenda, formularios enviados, tarifas, frases e historial BZ/de pruebas. Conserva la configuración base de BZ.",', 'description: "Deja la aplicación vacía de datos de negocio y contenido creado: alumnado, personas de prueba, enseñanza, clases, bonos, evaluaciones, medidas, CRM, marketing, estadísticas, misiones, agenda, formularios enviados, tarifas, frases, historial BZ y actividad de Feedback Online. Conserva la configuración base de BZ/Feedback.",', "reset full copy")
text = one(text, '  bz_points: "movimientos y acciones BZ",\n  medidas:', '  bz_points: "movimientos y acciones BZ",\n  feedback_online: "registros de Feedback Online",\n  medidas:', "reset feedback label")
