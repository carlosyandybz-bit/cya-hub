-- Expose only student-visible teaching media through the hardened portal
-- projection. Drive references remain private to the authenticated student
-- whose assignment owns the content.
  ...
  select jsonb_build_object(
    ...
    'assignments',coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'media',coalesce((
          select jsonb_agg(jsonb_build_object(
            'media_type',media.media_type,'provider',media.provider,
            'external_file_id',media.external_file_id,'title',media.title
