  ...
  loop
    ...
    if v_rate_name is not null then
      ...
      if v_rate_id is null then
        v_notes := concat_ws(E'\n',v_notes,'Bono/interés: ' || v_rate_name);
      end if;
    end if;

    if v_source is not null or v_notes is not null then
      update public.people p set
        ...
        notes = case
          when v_notes is null then p.notes
          when p.notes is null or btrim(p.notes)='' then v_notes
          when position(v_notes in p.notes)>0 then p.notes
          when v_before.duplicate_strategy='update' then concat_ws(E'\n',p.notes,v_notes)
          else p.notes end,
        updated_at = now()
    ...
    if v_contact_date is not null or v_inquiry is not null or v_reserved is not null or v_rate_id is not null or v_amount is not null then
      ...
      values(
        v_person_id,coalesce(v_contact_date,current_date),v_inquiry,coalesce(v_reserved,false),v_rate_id,v_amount,'unknown',(select auth.uid())
      )
      on conflict(person_id) do update set
        contact_date = case
          when v_before.duplicate_strategy='update' then coalesce(v_contact_date,public.crm_profiles.contact_date)
          else public.crm_profiles.contact_date end,
        inquiry = case
          when v_before.duplicate_strategy='update' or public.crm_profiles.inquiry is null then coalesce(v_inquiry,public.crm_profiles.inquiry)
          else public.crm_profiles.inquiry end,
        reserved = case
          when v_before.duplicate_strategy='update' and v_reserved is not null then v_reserved
          else public.crm_profiles.reserved end,
        rate_id = case
          when v_before.duplicate_strategy='update' or public.crm_profiles.rate_id is null then coalesce(v_rate_id,public.crm_profiles.rate_id)
          else public.crm_profiles.rate_id end,
        quoted_amount_cents = case
          when v_before.duplicate_strategy='update' or public.crm_profiles.quoted_amount_cents is null then coalesce(v_amount,public.crm_profiles.quoted_amount_cents)
          else public.crm_profiles.quoted_amount_cents end,
        updated_at=now();
    end if;
  end loop;
