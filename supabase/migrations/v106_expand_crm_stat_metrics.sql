...
begin
  ...
  if p_metric_key not in ('people_count','percentage_total','average_age','reserved_count','reservation_rate','next_class_count','questionnaire_pending_count','registered_count','registered_rate','total_reservations','average_reservations','people_with_cancellations','total_cancellations') then raise exception 'invalid metric'; end if;
