-- CYA Hub · v51 · Separación de evaluación inicial y revisión post-clase
--
-- Una misma clase/alumno/contexto puede y debe tener dos sesiones distintas:
-- - evaluation_kind='initial' durante la clase;
-- - evaluation_kind='class' después del cierre administrativo.
--
-- La versión anterior imponía unicidad sin evaluation_kind y las RPC podían
-- reutilizar accidentalmente la sesión inicial como revisión post-clase.

begin;

drop index if exists public.evaluation_sessions_one_class_context_uidx;
create unique index evaluation_sessions_one_class_context_uidx
...
begin
  ...
  loop
    ...
    values(
      ...
      v_progress.aptitude_term_id,'class',v_progress.effective_score,null,(select auth.uid()),
...
begin
  ...
  -- Compatibilidad con historiales anteriores al cutover: una evaluación ya
  -- completada del mismo estilo/rol sigue siendo baseline válida.
  if not exists(
    select 1
    from public.evaluation_sessions s
