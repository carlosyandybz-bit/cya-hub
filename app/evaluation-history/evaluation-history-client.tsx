...
type EvaluationRow = { id:number; session_id:number|null; class_id:number|null; style_term_id:number; role_term_id:number; level_term_id:number; aptitude_term_id:number; evaluation_kind:string; score:number; note:string|null; answer_label:string|null; reviewed_at:string|null; answer_scale_term_id:number|null; descriptor_id:number|null; created_at:string };
...
export default function EvaluationHistoryClient(){
  ...
  const loadBase=useCallback(async()=>{
    ...
    const firstError=profileResult.error||peopleResult.error||termResult.error||milestoneResult.error||descriptorResult.error||contentResult.error||recommendationResult.error;
    ...
    setStudents(nextStudents);setTerms((termResult.data??[]) as Term[]);setMilestones((milestoneResult.data??[]) as Milestone[]);setDescriptors((descriptorResult.data??[]) as Descriptor[]);setContents((contentResult.data??[]) as TeachingContent[]);setRecommendations((recommendationResult.data??[]) as Recommendation[]);
  ...
  const loadPerson=useCallback(async(id:number)=>{
    ...
    const [sessionResult,evaluationResult,progressResult,eventResult]=await Promise.all([
      ...
      client.from("student_evaluations").select("id,session_id,class_id,style_term_id,role_term_id,level_term_id,aptitude_term_id,evaluation_kind,score,note,answer_label,reviewed_at,answer_scale_term_id,descriptor_id,created_at").eq("person_id",id).order("created_at",{ascending:true}),
    ...
    const firstError=sessionResult.error||evaluationResult.error||progressResult.error||eventResult.error;
    if(firstError){setError(firstError.message);setBusy("");return;}
    const nextSessions=(sessionResult.data??[]) as SessionRow[],nextProgress=(progressResult.data??[]) as ProgressRow[],nextEvents=(eventResult.data??[]) as ClassEvent[];
    const sessionIds=nextSessions.map((row)=>row.id),classIds=[...new Set([...nextSessions.map((row)=>row.class_id),...nextEvents.map((row)=>row.class_id)].filter((value):value is number=>Boolean(value)))];
    ...
    setSessions(nextSessions);setEvaluations((evaluationResult.data??[]) as EvaluationRow[]);setProgress(nextProgress);setClassEvents(nextEvents);setDecisions((decisionResult.data??[]) as DecisionRow[]);setClasses((classResult.data??[]) as ClassRow[]);
  ...
  const selectedEvents=selectedSession?.class_id?classEvents.filter((row)=>row.class_id===selectedSession.class_id):[];
  const selectedContentIds=new Set(selectedEvents.map((row)=>row.content_id));
  const selectedRecommendations=context?recommendations.filter((row)=>selectedContentIds.has(row.content_id)&&row.style_term_id===context.styleId&&row.role_term_id===context.roleId&&row.level_term_id===context.levelId):[];
  ...
  const currentEvaluationForAptitude=activeAptitudeId?currentRows.find((row)=>row.aptitude_term_id===activeAptitudeId)??null:null;
  ...
  return <main className={styles.page}>
    ...
    {busy!=="person"&&busy!=="base"&&context?<>
      ...
      <div className={styles.mainGrid}>
        ...
        <section className={styles.card}><div className={styles.sectionHead}><div><span>Parámetro</span><h2>{activeAptitudeId?termLabel(activeAptitudeId):"Selecciona una aptitud"}</h2></div>{currentProgressForAptitude?<b>{currentProgressForAptitude.effective_score}/100</b>:null}</div>{activeAptitudeId?<><div className={styles.aptitudeStats}><div><span>Valor en la fecha</span><strong>{currentEvaluationForAptitude?.score??"—"}</strong></div><div><span>Respuesta del profesor</span><strong>{currentEvaluationForAptitude?.answer_label??(currentEvaluationForAptitude?"Histórico":"—")}</strong></div><div><span>Progreso actual</span><strong>{currentProgressForAptitude?.effective_score??"—"}</strong></div><div><span>Estado de revisión</span><strong>{currentEvaluationForAptitude?.reviewed_at?"Revisado":currentEvaluationForAptitude?"Legado":"—"}</strong></div></div><TrendChart points={trendPoints}/>{currentProgressForAptitude?.pending_milestone_id?<div className={styles.pendingMilestone}><Clock3/><div><span>Hito pendiente</span><strong>{milestones.find((item)=>item.id===currentProgressForAptitude.pending_milestone_id)?.label??"Pendiente de revisión"}</strong></div></div>:<div className={styles.okBox}><CheckCircle2/><span>Sin hitos pendientes en esta aptitud.</span></div>}</>:<div className={styles.empty}>No hay aptitudes disponibles.</div>}</section>
      </div>
