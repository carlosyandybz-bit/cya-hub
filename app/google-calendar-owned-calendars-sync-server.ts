...
export type OwnedCalendarSyncMetrics = {
  calendars: number;
  imported: number;
  updated: number;
  removed: number;
...
export async function syncOwnedGoogleCalendars(
  accessToken: string,
  connection: CalendarConnectionRow,
): Promise<OwnedCalendarSyncMetrics> {
  ...
  const metrics: OwnedCalendarSyncMetrics = { calendars: 0, imported: 0, updated: 0, removed: 0 };
  ...
  for (const calendar of calendars) {
    ...
    for (const event of events) {
      ...
      if (event.extendedProperties?.private?.cyaHub === "1") continue;
      ...
      const data = {
        ...
        metadata: {
          ...
          calendar_foreground: calendar.foregroundColor ?? null,
          google_color_id: calendar.colorId ?? null,
          read_only: true,
        },
      };
