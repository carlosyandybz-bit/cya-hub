...
import { syncOwnedGoogleCalendars } from "../../../google-calendar-owned-calendars-sync-server";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const ownedCalendars = await syncOwnedGoogleCalendars(accessToken, connection);
    return NextResponse.json({ ok: true, metrics: { ...metrics, ownedCalendars } }, { headers: { "cache-control": "no-store" } });
  } catch (error) {
