...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    if (!context || Number(context.request_id) !== payload.requestId || Number(context.person_id) !== payload.personId) {
      return NextResponse.json({ error: "La solicitud de Feedback ya no admite este vídeo." }, { status: 403 });
    }
