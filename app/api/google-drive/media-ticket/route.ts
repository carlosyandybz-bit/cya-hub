...
import { driveServerConfigured, signMediaTicket, userCanAccessTeachingMedia } from "../../../google-drive-server";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!(await userCanAccessTeachingMedia(accessToken, fileId))) return NextResponse.json({ error: "No tienes permiso para ver este archivo." }, { status: 403 });
