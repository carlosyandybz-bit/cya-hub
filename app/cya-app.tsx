...
import {
  AlertTriangle, Archive, ArrowRight, BarChart3, Bell, BellRing, BookOpen, CalendarDays, CheckCircle2, ChevronDown, ChevronRight, ClipboardCheck,
  Dumbbell, Eye, EyeOff, GitBranch, GraduationCap, House,
  LibraryBig, Link2, LockKeyhole, Megaphone,
  Pencil, Play, Plus, Search, Sparkles, Target, TrendingUp, UserRound, UsersRound,
  WalletCards, X,
} from "lucide-react";
...
import { StaffMissionsView } from "./staff-missions-view";
import { missionDestination } from "./mission-routing";
import type { StudentDetailTab } from "./student-detail-navigation";
import { StatisticsView } from "./statistics-view";
import { StudentExercisesPanel } from "./student-exercises-panel";
import { LiveStudentExercises } from "./live-student-exercises";
import { isLiveSearchActive } from "./live-search-state";
import { staffPersonMatches, staffPrimaryName, staffRealNameWhenAliased } from "./staff-person-name";
import type { ExperienceContext, IdentityContext, Mission } from "./v14-types";

const TeachingGraph = lazy(() => import("./teaching-graph").then((module) => ({ default: module.TeachingGraph })));

type View = "home" | "students" | "classes" | "credits" | "agenda" | "live" | "teaching" | "marketing" | "statistics" | "academy" | "missions" | "admin" | "profile" | "preferences" | "notifications";
...
type Person = {
  ...
  internal_alias: string | null; crm_stage: string; active: boolean;
};
...
type ClassNote = { id: number; class_id: number; person_id: number | null; content_id: number | null; body: string; visibility_scope: "internal" | "student"; created_at: string };
...
type StudentPortalSnapshot = {
  ...
  assignments: Array<{
    ...
    correction_guidance: string | null; is_mandatory?: boolean; assignment_status: string; current_frequency: number | null;
...
const nav = [
  ...
  ["academy", "Academia", GraduationCap],
] as const;
...
function StudentsView({ students, query, setQuery, add, open, schedule, credit }: { students: Person[]; query: string; setQuery: (v: string) => void; add: () => void; open: (p: Person) => void; schedule: (p: Person) => void; credit: (p: Person) => void }) {
  const filtered = useMemo(() => {
    return students.filter((student) => staffPersonMatches(student, query));
  }, [students, query]);
  return <>
    ...
    <label className="search"><Search /><input type="search" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Buscar alias, nombre, teléfono o email" /></label>
    {filtered.length ? <div className="student-list">{filtered.map((student) =>
      <article className="student-row" key={student.id} data-person-id={student.id}>
        <button className="student-row-main" onClick={() => open(student)}>
          <span className="avatar"><UserRound /></span><span className="student-main"><strong>{staffPrimaryName(student)}</strong><span>{staffRealNameWhenAliased(student) || student.phone || student.email || "Sin datos de contacto"}</span></span>
...
function namesFor(ids: number[], students: Person[]) {
  return ids.map((id) => { const student = students.find((person) => person.id === id); return student ? staffPrimaryName(student) : "Alumno"; }).join(" + ");
}

function studentNameFor(students: Person[], personId: number) {
  const student = students.find((person) => person.id === personId);
  return student ? staffPrimaryName(student) : "Alumno";
}
...
function assignmentOptionsOutsideClass(contentType: string) {
  return contentType === "correction" ? [["pending", "Pendiente de corrección"], ["corrected", "Corregido"]] as const : assignmentOptions(contentType);
}
...
function FinishClassModal({ item, students, credits, library, close, finished }: { item: ClassItem; students: Person[]; credits: CreditItem[]; library: TeachingContent[]; close: () => void; finished: () => Promise<void> }) {
  ...
  function ownerLabel(grant: CreditItem) {
    ...
    const person = students.find((candidate) => candidate.id === personId);
    return person ? staffPrimaryName(person) : "Alumno";
  }
  ...
  function renderRegularization(grant: CreditItem) {
  ...
  return <div className="backdrop"><section className="modal finish-modal" role="dialog" aria-modal="true">
    ...
    <form className="modal-body" onSubmit={submit}>
      ...
      {item.class_type === "pair" ? <section className="finish-person finish-pair-credit"><strong>{namesFor(classPersonIds, students)}</strong><label className="field"><span>Bono de pareja</span><select value={selectedGrantId} disabled={billingMode === "direct"} onChange={(event) => chooseGrant(classPersonIds[0], event.target.value)}><option value="">Sin bono</option>{pairAvailable.map((grant) => { const balance = creditBalance(grant), shortfall = Math.max(0, manualDuration - balance); return <option key={grant.id} value={grant.id}>{grant.label || "Bono de pareja"} · {minutesLabel(balance)}{expiryLabel(grant)}{shortfall ? ` · faltarán ${minutesLabel(shortfall)}` : ""}</option>; })}</select></label>{selectedGrant ? renderRegularization(selectedGrant) : null}</section> : <div className="finish-list">{item.class_participants.map((participant) => {
        ...
        return <section className="finish-person" key={participant.person_id}><strong>{student ? staffPrimaryName(student) : "Alumno"}</strong><label className="field"><span>Bono</span><select value={grantIds[participant.person_id] || ""} disabled={billingMode === "direct"} onChange={(event) => chooseGrant(participant.person_id, event.target.value)}><option value="">Sin bono</option>{available.map((grant) => { const balance = creditBalance(grant), shortfall = Math.max(0, manualDuration - balance); return <option key={grant.id} value={grant.id}>{grant.label || "Bono individual"} · {minutesLabel(balance)}{expiryLabel(grant)}{shortfall ? ` · faltarán ${minutesLabel(shortfall)}` : ""}</option>; })}</select></label>{selected ? renderRegularization(selected) : null}</section>;
      })}</div>}
...
function ManualClassDraft({ students, close, created, refresh }: { students: Person[]; close: () => void; created: (id: number) => Promise<void>; refresh: () => Promise<void> }) {
  ...
  return <><div className="backdrop" onMouseDown={(event) => event.target === event.currentTarget && close()}><section className="modal" role="dialog" aria-modal="true">
    ...
    <form className="modal-body" onSubmit={submit}><div className="segmented"><button type="button" className={type === "individual" ? "active" : ""} onClick={() => setType("individual")}>Individual</button><button type="button" className={type === "pair" ? "active" : ""} onClick={() => setType("pair")}>Pareja</button></div><div className="fields-2"><div className="field"><span>Alumno *</span><select name="student_1" required value={firstId} onChange={(event) => setFirstId(event.target.value)}><option value="" disabled>Seleccionar</option>{students.map((student) => <option key={student.id} value={student.id}>{staffPrimaryName(student)}</option>)}</select><button type="button" className="text-button" onClick={() => setQuickSlot(1)}><Plus size={15}/> Crear alumno provisional</button></div>{type === "pair" ? <div className="field"><span>Segundo alumno *</span><select name="student_2" required value={secondId} onChange={(event) => setSecondId(event.target.value)}><option value="" disabled>Seleccionar</option>{students.map((student) => <option key={student.id} value={student.id}>{staffPrimaryName(student)}</option>)}</select><button type="button" className="text-button" onClick={() => setQuickSlot(2)}><Plus size={15}/> Crear alumno provisional</button></div> : null}</div><p className="modal-intro">CYA reutilizará fecha, duración y el contexto de baile que ya conozca. Después solo te pedirá lo que realmente falte.</p>{error ? <p className="error">{error}</p> : null}<div className="actions"><button className="btn ghost" type="button" onClick={close}>Cancelar</button><button className="btn" disabled={busy}><ArrowRight size={17} /> {busy ? "Creando…" : "Continuar"}</button></div></form>
...
function ClassPreparationStage({ item, classes, students, assignments, terms, refresh, notify, back, editData }: { item: ClassItem; classes: ClassItem[]; students: Person[]; assignments: ContentAssignment[]; terms: CatalogTerm[]; refresh: () => Promise<void>; notify: (message:string) => void; back: () => void; editData: () => void }) {
  ...
  return <div className="class-workflow-page"><header className="workflow-head"><button className="icon-btn" onClick={back} aria-label="Volver al centro de clases">‹</button><div><p className="eyebrow">2 · Preparar</p><h1>{namesFor(personIds,students)}</h1><p>Ubica al alumno antes de empezar a bailar.</p></div><button className="btn ghost workflow-edit-data" onClick={editData}>Editar datos</button></header><div className="workflow-stepbar"><span>Datos</span><span className="active">Preparar</span><span>Dar clase</span><span>Resumen</span></div>
    ...
    {error ? <p className="error">{error}</p> : null}<div className="workflow-footer"><button className="btn ghost" onClick={back}>Salir</button><button className="btn workflow-primary" data-mission-highlight="class-action" onClick={() => void begin()} disabled={busy}><Play /> {busy ? "Abriendo…" : "Empezar clase"}</button></div>
  </div>;
}
...
function ClassFinalSummary({ item, students, library, refresh, notify, done, back }: { item: ClassItem; students: Person[]; library: TeachingContent[]; refresh: () => Promise<void>; notify: (message:string) => void; done: () => void; back: () => void }) {
  ...
  return <div className="class-workflow-page final-summary"><header className="workflow-head"><button className="icon-btn" onClick={back} aria-label="Volver">‹</button><div><p className="eyebrow">Cierre pedagógico</p><h1>Resumen de la clase</h1><p>{namesFor(personIds,students)} · {dateLabel(item.scheduled_start_at)}</p></div></header><div className="workflow-stepbar"><span>Datos</span><span>Preparar</span><span>Dar clase</span><span className="active">Resumen</span></div>
    ...
    <section className="card pad workflow-card"><div className="card-head"><h2>Trabajado hoy</h2><span>{newItems.length + editedItems.length + exercises.length}</span></div><div className="summary-neutral">{newItems.map((event) => renderEvent(event,'neutral'))}{editedItems.map((event) => renderEvent(event,'neutral'))}{exercises.map((event) => renderEvent(event,'neutral'))}{!newItems.length && !editedItems.length && !exercises.length ? <p className="modal-intro">No se añadieron contenidos nuevos.</p> : null}</div></section>
    <section className="card pad workflow-card"><div className="card-head"><div><p className="eyebrow">Opcional</p><h2>Evaluación actual</h2></div><span>Alumno</span></div><p className="modal-intro">Los parámetros pertenecen al alumno. Puedes revisar hitos ahora o cerrar la clase sin cambiar nada.</p>{item.class_participants.map((participant)=>{const person=students.find((row)=>row.id===participant.person_id);return db?<ContextEvaluationPanel key={participant.person_id} client={db} personId={participant.person_id} personName={person ? staffPrimaryName(person) : 'Alumno'} classId={item.id} styleTermId={item.style_term_id} roleTermId={participant.role_term_id} levelTermId={participant.level_term_id}/>:null;})}</section>
    ...
    <section className="card pad workflow-card"><div className="card-head"><div><p className="eyebrow">Documentación</p><h2>Baile final y archivos de clase</h2></div><label className="btn ghost video-add"><Plus /> Añadir<input type="file" accept="image/*,video/*" multiple disabled={busy} onChange={(event) => { addMedia(event.target.files); event.currentTarget.value=''; }} /></label></div>{media.length ? <div className="summary-media-list">{media.map((draft) => <article key={draft.id}><div><strong>{draft.file.name}</strong><span>{draft.saved?'Guardado':'Pendiente'}</span></div><label className="field"><span>Tipo</span><select value={draft.kind} disabled={draft.saved} onChange={(event) => setMedia((current) => current.map((row) => row.id===draft.id ? {...row,kind:event.target.value as MediaDraft['kind']} : row))}><option value="final_dance">Baile final</option><option value="class_document">Documento de clase</option></select></label>{item.class_type==='pair' ? <label className="field"><span>Disponible para</span><select value={draft.audience} disabled={draft.saved} onChange={(event) => setMedia((current) => current.map((row) => row.id===draft.id ? {...row,audience:event.target.value} : row))}><option value="both">Ambos</option>{personIds.map((personId) => <option key={personId} value={personId}>{studentNameFor(students, personId)}</option>)}</select></label> : null}<button className="icon-btn" type="button" aria-label="Quitar archivo" disabled={draft.saved} onClick={() => setMedia((current) => current.filter((row) => row.id!==draft.id))}><X /></button></article>)}</div> : <p className="modal-intro">Opcional. El baile final queda vinculado a esta clase para seguir la evolución.</p>}</section>
    {error ? <p className="error">{error}</p> : null}<div className="workflow-footer"><button className="btn ghost" onClick={back}>Volver</button><button className="btn" data-mission-highlight="pedagogy-close" onClick={() => void closeSummary()} disabled={busy}><CheckCircle2 /> {busy?'Cerrando…':'Cerrar y enviar al alumno'}</button></div>
  </div>;
}
...
function LiveSession({ item, classes, students, credits, terms, library, relations, refresh, notify, exit }: { item: ClassItem; classes: ClassItem[]; students: Person[]; credits: CreditItem[]; terms: CatalogTerm[]; library: TeachingContent[]; relations: TeachingRelation[]; refresh: () => Promise<void>; notify: (message: string) => void; exit: () => void }) {
  ...
  const [search,setSearch]=useState(""), [searchFocused,setSearchFocused]=useState(false), [contentFilter,setContentFilter]=useState<"correction"|"explanation"|"exercise"|"sequence">("correction"), [searchResults,setSearchResults]=useState<LiveClassSearchResult[]>([]), [searchLoading,setSearchLoading]=useState(false), [searchError,setSearchError]=useState(""), [showCorrected,setShowCorrected]=useState(true), [showLearningHistory,setShowLearningHistory]=useState(true), [liveTab,setLiveTab]=useState<"work"|"context"|"evaluation">("work");
  ...
  const [quickFrequency,setQuickFrequency]=useState<number|null>(null), [quickInfluence,setQuickInfluence]=useState<number|null>(null), [quickObservation,setQuickObservation]=useState(""), [quickObservationVisibility,setQuickObservationVisibility]=useState<"internal"|"student">("internal");
  ...
  const correctionStatusPriority=(status:string)=>status==='in_correction'?0:status==='pending'?1:status==='corrected'?2:3;
  ...
  const allSuggestedNext=[...connectedNext,...freeNext.filter((content) => !connectedNext.some((candidate) => candidate.id===content.id))], suggestedNext=showAllSuggestions?allSuggestedNext:allSuggestedNext.slice(0,3), connectedSuggestionIds=new Set(connectedNext.map((content) => content.id));
  const shouldShowSearch=Boolean(search.trim()), searchActive=isLiveSearchActive({focused:searchFocused,query:search,loading:searchLoading,resultCount:searchResults.length}), searchPersonId=participant?.person_id ?? null;
  useEffect(() => {
    let alive=true;
    const timer=window.setTimeout(async () => {
      ...
      const result=await db.rpc('search_class_teaching_content',{p_class_id:item.id,p_person_id:searchPersonId,p_query:search.trim(),p_content_type:null,p_limit:30});
      if (!alive) return;
  ...
  },[contextReady,item.id,search,searchPersonId,shouldShowSearch]);
  async function createQuickContent(titleOverride?:string,typeOverride?:typeof quickType) {
    const title=(titleOverride ?? quickTitle).trim(), createdType=typeOverride ?? quickType;
    if (!db || !participant || !title || !contextReady) return;
    setBusy('quick-create');
    const result=createdType==='correction'
      ? await db.rpc('create_class_correction_compact',{p_class_id:item.id,p_person_id:participant.person_id,p_title:title,p_frequency:typeOverride?null:quickFrequency,p_importance:typeOverride?null:quickInfluence,p_observation:typeOverride?null:(quickObservation.trim()||null),p_observation_visibility:quickObservationVisibility})
      : await db.rpc('create_quick_class_content',{p_class_id:item.id,p_person_id:participant.person_id,p_content_type:createdType,p_title:title});
    if (result.error) notify(result.error.message);
    else { setQuickTitle(''); if(titleOverride)setSearch(''); if(createdType==='correction'){setQuickFrequency(null);setQuickInfluence(null);setQuickObservation('');setQuickObservationVisibility('internal');} await Promise.all([refresh(),loadLive()]); notify(createdType==='correction'?'Corrección pendiente añadida.':`${teachingKindLabels[createdType]} apuntada para completar después.`); }
    setBusy('');
  }
  ...
  async function saveContentNote(event:FormEvent<HTMLFormElement>,contentId:number) {
  ...
  function renderCorrectionSummary(assignment:ContentAssignment){ return <CorrectionQuickControls status={assignment.assignment_status} statusOptions={correctionStates} frequency={assignment.current_frequency} importance={assignment.current_importance} measurementMode={assignment.snapshot_measurement_mode} disabled={busy===`correction-${assignment.id}`} onStatus={(status)=>updateCorrection(assignment,{status})} onFrequency={(frequency)=>updateCorrection(assignment,{frequency})} onImportance={(importance)=>updateCorrection(assignment,{importance})}/>; }
  function renderCorrectionControls(assignment:ContentAssignment){ return <div className="p0g-detail-controls">{renderContentNote(assignment.content_id)}</div>; }
  ...
  const balanceMinutes=credits.filter((grant) => grant.status==='active' && grant.credit_grant_members.some((member) => member.person_id===activePersonId)).reduce((sum,grant)=>sum+Math.max(0,creditBalance(grant)),0);
  return <div className="live-overlay"><div className="live-sticky"><header className="live-top"><div className="live-title"><span className="live-dot"/><div><span>EN CLASE</span><strong>{style?.label || 'Clase activa'}</strong><small>{minutesLabel(item.duration_minutes)}</small></div></div><div className="live-actions"><button className="btn" onClick={() => setFinishOpen(true)}><CheckCircle2/> Terminar</button><button className="icon-btn live-exit" onClick={exit} aria-label="Volver al centro de clases"><X/></button></div></header></div>
    <main className="live-body">{item.class_participants.length>1 ? <div className="participant-tabs">{item.class_participants.map((p) => <button key={p.person_id} className={activePersonId===p.person_id?'active':''} onClick={() => setActivePersonId(p.person_id)}>{studentNameFor(students, p.person_id)}</button>)}</div> : null}<section className="student-context card"><div className="student-context-main"><span className="avatar live-student-avatar"><UserRound/></span><div><p className="eyebrow">Alumno</p><h2>{student ? staffPrimaryName(student) : 'Alumno'}</h2><p>{roleTerm?.label || 'Rol pendiente'} · {levelTerm?.label || 'Nivel pendiente'}</p></div></div><details className="student-context-details"><summary>Información</summary><div><p><strong>Contexto:</strong> {style?.label || 'Sin estilo'} · {roleTerm?.label || 'Rol pendiente'} · {levelTerm?.label || 'Nivel pendiente'}</p><p><strong>Saldo disponible:</strong> {minutesLabel(balanceMinutes)}</p>{prepProfile?.goals?<p><strong>Objetivo:</strong> {prepProfile.goals}</p>:null}{prepProfile?.teacher_notes?<p><strong>Notas útiles:</strong> {prepProfile.teacher_notes}</p>:null}{previousClass?<p><strong>Última clase:</strong> {dateLabel(previousClass.scheduled_start_at)}</p>:null}</div></details></section>
      <nav className="live-work-tabs p0f-primary-tabs"><button className={liveTab==='work'?'active':''} onClick={() => setLiveTab('work')}><BookOpen/> Trabajo</button><button className={liveTab==='context'?'active':''} onClick={() => setLiveTab('context')}><Sparkles/> Contexto</button><button className={liveTab==='evaluation'?'active':''} onClick={() => setLiveTab('evaluation')}><ClipboardCheck/> Evaluación</button></nav>
      <div className="live-search-area p0f-sticky-search" data-search-active={searchActive}><label className="live-search"><Search/><input type="search" aria-label="Buscar contenido de clase" value={search} onFocus={() => setSearchFocused(true)} onBlur={() => setSearchFocused(false)} onChange={(event) => setSearch(event.target.value)} onKeyDown={(event) => { if(event.key==='Enter' && !event.nativeEvent.isComposing && search.trim()){event.preventDefault();void createQuickContent(search,contentFilter);} }} placeholder="Buscar o escribir + Enter para capturar…"/><kbd>Enter</kbd></label></div>
      {!searchActive ? <nav className="live-search-kinds p0f-content-tabs">{([['correction','Correcciones'],['explanation','Explicaciones'],['exercise','Ejercicios'],['sequence','Secuencias']] as const).map(([value,label]) => <button key={value} className={contentFilter===value?'active':''} onClick={() => {setContentFilter(value);setQuickType(value);}}>{label}</button>)}</nav> : null}{syncError ? <p className="error">{syncError}</p> : null}
      <section className="live-unified-search card"><details className="quick-content-create" data-search-first-option="true" onToggle={(event) => {if(event.currentTarget.open){setQuickType(contentFilter);setQuickTitle((current)=>current||search.trim());}}}><summary aria-label={search.trim()?`Crear nuevo contenido: ${search.trim()}`:"Crear nuevo contenido"}><Plus/> {search.trim()?<>Crear nuevo: “{search.trim()}”</>:"Crear nuevo"}</summary><div><select value={quickType} onChange={(event) => setQuickType(event.target.value as typeof quickType)}><option value="correction">Corrección</option><option value="explanation">Explicación</option><option value="exercise">Ejercicio</option><option value="sequence">Secuencia</option></select><input value={quickTitle} onChange={(event) => setQuickTitle(event.target.value)} placeholder="Título corto"/>{quickType==='correction'?<div className="p0g-create-correction"><div className="p0g-create-measures"><label>Frec.<select aria-label="Frecuencia inicial" value={quickFrequency??''} onChange={(event)=>setQuickFrequency(event.target.value===''?null:Number(event.target.value))}><option value="">Sin valorar</option>{frequencyLevels.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label><label>Infl.<select aria-label="Influencia inicial" value={quickInfluence??''} onChange={(event)=>setQuickInfluence(event.target.value===''?null:Number(event.target.value))}><option value="">Sin valorar</option>{influenceLevels.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label></div><div className="p0g-create-observation"><label>Observación opcional<textarea rows={2} value={quickObservation} onChange={(event)=>setQuickObservation(event.target.value)} placeholder="Algo concreto de este alumno…"/></label><select aria-label="Visibilidad de la observación inicial" value={quickObservationVisibility} onChange={(event)=>setQuickObservationVisibility(event.target.value as 'internal'|'student')}><option value="internal">🔒 Profesores</option><option value="student">👁 Alumno</option></select></div><p className="p0g-create-hint"><strong>Pend.</strong> es automático. Si dejas F/I/observación vacíos, permanecen sin valorar.</p></div>:null}<button className="btn" onClick={() => void createQuickContent()} disabled={!quickTitle.trim() || busy==='quick-create'}>{busy==='quick-create'?'Guardando…':'Crear'}</button></div><small>CYA hereda alumno, estilo, rol y nivel. Lo nuevo queda interno hasta que corresponda.</small></details>{shouldShowSearch ? <div className="unified-results"><div className="unified-result-head"><strong>Resultados</strong><span>{searchLoading?'…':searchResults.length}</span></div>{searchError ? <p className="error live-search-error">{searchError}</p> : null}{searchLoading ? <div className="compact-empty"><span className="spinner"/><span>Buscando en el contexto de {student ? staffPrimaryName(student) : 'este alumno'}…</span></div> : searchResults.map((result) => { const type=result.content_type, content=library.find((row) => row.id===result.content_id), assignment=personAssignments.find((row) => row.content_id===result.content_id), exerciseEvent=exerciseEvents.find((row) => row.content_id===result.content_id), partnerBlocked=type==='exercise'&&Boolean(content?.requires_partner)&&item.class_participants.length<2; const statusLabel=partnerBlocked?'Necesita pareja':type==='exercise' ? (exerciseEvent?.event_type==='exercise_completed'?'Realizado':exerciseEvent?.event_type==='exercise_active'?'Activo':exerciseEvent?.event_type==='exercise_pending'?'Pendiente':'Disponible') : assignment ? (assignmentOptions(type).find(([value]) => value===assignment.assignment_status)?.[1] || assignment.assignment_status) : result.ready?'Compatible con esta clase':'Incompleta · solo profesores'; return <article className={`unified-result ${assignment||exerciseEvent?'assigned':''}`} data-kind={type} key={`search-${result.content_id}`}><span className="content-kind">{teachingKindLabels[type]}</span><div><strong>{result.title}</strong><small>{statusLabel}</small></div><div className="unified-result-actions">{type==='correction' && assignment ? <ContentStatusControl value={assignment.assignment_status} options={correctionStates} disabled={Boolean(busy)} onChange={(status)=>updateCorrection(assignment,{status})}/> : ['explanation','sequence'].includes(type) && assignment ? assignment.assignment_status==='explained' ? <button className="btn ghost" onClick={() => void recordEvent(result.content_id,'reviewed')}>Repasar</button> : <button className="btn" onClick={() => void updateLearning(assignment,'explained')}>Explicada</button> : type==='exercise' ? exerciseEvent?.event_type==='exercise_completed' ? <CheckCircle2/> : exerciseEvent?.event_type==='exercise_active' ? <button className="btn" onClick={() => void recordEvent(result.content_id,'exercise_completed')}>Realizado</button> : <button className="btn" onClick={() => void recordEvent(result.content_id,'exercise_active')}>{exerciseEvent?'Activar':'Usar'}</button> : !assignment && result.ready && content ? <button className="btn" disabled={busy===`assign-${result.content_id}`} onClick={() => void assignContent(content)}><Plus/> Añadir</button> : !assignment ? <span className="badge">Completar después</span> : <CheckCircle2/>}</div></article>; })}{!searchLoading && !searchError && !searchResults.length ? <div className="compact-empty"><Search/><span>No hay coincidencias para este alumno, estilo, rol y nivel.</span></div> : null}</div> : null}</section>
      {liveTab==='work' ? <section className={`live-workspace p0f-filter-${contentFilter}`}><article className="card live-card corrections-card"><div className="live-card-head"><div><p className="eyebrow">Prioridad</p><h2>Correcciones</h2></div><button className="text-button" onClick={() => setShowCorrected(!showCorrected)}>{showCorrected?'Solo pendientes':'Ver corregidas'}</button></div><div className="correction-list">{visibleCorrections.length ? visibleCorrections.map((assignment) => { const content=library.find((row) => row.id===assignment.content_id); return <TeachingContentCard key={assignment.id} kindTone="correction" className={`live-content-card ${Math.max(assignment.current_frequency ?? 0,assignment.current_importance ?? 0)>=75?'live-priority-high':Math.max(assignment.current_frequency ?? 0,assignment.current_importance ?? 0)>=50?'live-priority-medium':'live-priority-low'}`} kindLabel="Corrección" title={assignment.teaching_contents.title} summary={assignment.teaching_contents.summary} subtitle={assignment.teaching_contents.description} statusLabel={correctionStateShortLabel(assignment.assignment_status)} statusTone={assignment.assignment_status==='corrected'?'success':assignment.assignment_status==='in_correction'?'warning':'default'} description={assignment.teaching_contents.description} correctionGuidance={assignment.teaching_contents.correction_guidance} media={content?.teaching_content_media ?? []} actions={renderChangeMarker(assignment.content_id)} inlineControls={renderCorrectionSummary(assignment)}>{renderCorrectionControls(assignment)}</TeachingContentCard>; }) : <div className="compact-empty"><CheckCircle2/><span>{showCorrected?'No hay correcciones en el histórico.':'No hay correcciones pendientes.'}</span></div>}</div></article>
        <article className="card live-card learning-card"><div className="live-card-head"><div><p className="eyebrow">Explicaciones y secuencias</p><h2>Contenido</h2></div><button className="text-button" onClick={() => setShowLearningHistory(!showLearningHistory)}>{showLearningHistory?'Ver recientes':'Ver más'}</button></div><div className="guide-active">{visibleLearning.length ? visibleLearning.map((assignment) => { const content=library.find((row) => row.id===assignment.content_id); return <TeachingContentCard key={assignment.id} kindTone={assignment.teaching_contents.content_type as "explanation"|"sequence"} className="live-content-card" kindLabel={teachingKindLabels[assignment.teaching_contents.content_type]} title={assignment.teaching_contents.title} summary={assignment.teaching_contents.summary} subtitle={assignment.teaching_contents.description} statusLabel={assignmentOptions(assignment.teaching_contents.content_type).find(([value]) => value===assignment.assignment_status)?.[1] || assignment.assignment_status} statusTone={assignment.assignment_status==='explained'?'success':'warning'} description={assignment.teaching_contents.description} media={content?.teaching_content_media ?? []} actions={renderChangeMarker(assignment.content_id)} detailControls={<div className="p0g-detail-controls"><div className="p0g-detail-row"><ContentStatusControl value={assignment.assignment_status} options={assignmentOptions(assignment.teaching_contents.content_type)} disabled={Boolean(busy)} onChange={(status)=>updateLearning(assignment,status)}/>{assignment.assignment_status==='explained'?<button className="btn ghost live-mini-action" onClick={()=>void recordEvent(assignment.content_id,'reviewed')}>Repasar</button>:null}</div>{renderContentNote(assignment.content_id)}</div>}/>; }) : <div className="compact-empty"><LibraryBig/><span>No hay explicaciones o secuencias asignadas.</span></div>}</div></article>
        <LiveStudentExercises client={db!} personId={activePersonId} classId={item.id} participantCount={item.class_participants.length} classEvents={events} notify={notify} onChanged={loadLive} /></section> : null}
      {liveTab==='evaluation' && db && participant ? <section className="live-evaluation-workspace"><article className="card live-card"><ContextEvaluationPanel client={db} personId={activePersonId} personName={student ? staffPrimaryName(student) : 'Alumno'} classId={item.id} styleTermId={item.style_term_id} roleTermId={participant.role_term_id} levelTermId={participant.level_term_id} onCompleted={loadLive} /></article></section> : null}
      {liveTab==='context' ? <section className="live-context-grid"><article className="card live-card live-context-card"><div className="live-card-head"><div><p className="eyebrow">Contexto pedagógico</p><h2>{student ? staffPrimaryName(student) : 'Alumno'}</h2></div><Sparkles/></div>{previousClass ? <div className="context-strip"><span>Última clase</span><strong>{dateLabel(previousClass.scheduled_start_at)} · {minutesLabel(previousClass.duration_minutes)}</strong></div> : null}{prepProfile?.health_notes ? <div className="prepare-alert"><AlertTriangle/><div><strong>A tener en cuenta</strong><span>{prepProfile.health_notes}</span></div></div> : null}{prepProfile?.teacher_notes ? <div className="prepare-info"><strong>Notas del profesor</strong><p>{prepProfile.teacher_notes}</p></div> : null}{prepProfile?.goals ? <div className="prepare-info"><strong>Objetivo</strong><p>{prepProfile.goals}</p></div> : null}<div className="context-mini-grid"><div><strong>{corrections.filter((assignment) => assignment.assignment_status==='pending').length}</strong><span>correcciones pendientes</span></div><div><strong>{recentLearningContext.length}</strong><span>contenidos recientes</span></div></div>{corrections.filter((assignment) => assignment.assignment_status==='pending').length ? <div className="prepare-list"><strong>Mirar hoy</strong>{corrections.filter((assignment) => assignment.assignment_status==='pending').slice(0,4).map((assignment) => <span key={assignment.id}>{assignment.teaching_contents.title}</span>)}</div> : null}{recentLearningContext.length ? <div className="prepare-list"><strong>Últimas explicaciones y secuencias</strong>{recentLearningContext.map((assignment) => <span key={assignment.id}>{assignment.teaching_contents.title} · {assignmentOptions(assignment.teaching_contents.content_type).find(([value]) => value===assignment.assignment_status)?.[1] || assignment.assignment_status}</span>)}</div> : null}</article><article className="card live-card live-context-card"><div className="live-card-head"><div><p className="eyebrow">Para esta clase</p><h2>Decisiones del alumno</h2></div><span className="badge">{prepRequestsForPerson.length}</span></div>{prepRequestsForPerson.length ? <div className="request-list live-request-list">{prepRequestsForPerson.map((request) => { const requested=request.content_id ? library.find((content) => content.id===request.content_id) : null, canAdd=Boolean(requested && requested.active && requested.completion_status==='complete' && requested.publication_status==='published' && contentFitsContext(requested,item.style_term_id,participant?.role_term_id ?? null,participant?.level_term_id ?? null)); return <article key={request.id}><span>{request.request_type==='video'?'Vídeo':request.request_type==='focus'?'Quiere trabajar':request.request_type==='content'?'Contenido':'Mensaje'}</span><strong>{request.body || requested?.title || (request.external_file_id?'Vídeo adjunto':'Petición guardada')}</strong>{request.external_file_id ? <SecureDriveAsset fileId={request.external_file_id} mediaType="video" title="Vídeo para preparar la clase" controls className="request-video" /> : null}{requested && canAdd && !assignedContentIds.has(requested.id) ? <button className="btn ghost context-add" onClick={() => void assignContent(requested)}><Plus/> Añadir a esta clase</button> : null}</article>; })}</div> : <div className="compact-empty"><Sparkles/><span>No dejó indicaciones específicas para esta clase.</span></div>}</article><article className="card live-card live-context-card context-suggestions"><div className="live-card-head"><div><p className="eyebrow">Guía de hoy</p><h2>Siguiente contenido</h2></div><div className="recommendation-actions"><span className="badge">{allSuggestedNext.length}</span>{allSuggestedNext.length>3?<button type="button" className="text-button" onClick={()=>setShowAllSuggestions((value)=>!value)}>{showAllSuggestions?'Ver menos':'Ver más'}</button>:null}</div></div>{suggestedNext.length ? <div className="context-suggestion-list">{suggestedNext.map((content) => <article data-kind={content.content_type} key={content.id}><div><span>{connectedSuggestionIds.has(content.id)?'Siguiente por mapa':'Compatible'}</span><strong>{content.title}</strong><small>{teachingKindLabels[content.content_type]}</small></div><button className="btn" disabled={busy===`assign-${content.id}`} onClick={() => void assignContent(content)}><Plus/> Añadir</button></article>)}</div> : <div className="compact-empty"><GitBranch/><span>No hay otro contenido compatible desbloqueado.</span></div>}</article></section> : null}
...
function LiveClassView({ classes, students, credits, terms, library, relations, assignments, selectedClassId, targetHighlight, selectClass, refresh, notify, exit }: { classes: ClassItem[]; students: Person[]; credits: CreditItem[]; terms: CatalogTerm[]; library: TeachingContent[]; relations: TeachingRelation[]; assignments: ContentAssignment[]; selectedClassId: number | null; targetHighlight?: string; selectClass: (id: number | null) => void; refresh: () => Promise<void>; notify: (message: string) => void; exit: () => void }) {
  ...
  useEffect(() => {
    if (!selectedClassId || targetHighlight !== "pedagogy-close") return;
    const timer=window.setTimeout(() => setSummaryId(selectedClassId),0);
    return () => window.clearTimeout(timer);
  }, [selectedClassId, targetHighlight]);
  if (selected) {
  ...
  return <div className="backdrop" onMouseDown={(event) => event.target === event.currentTarget && close()}><section className="modal teaching-modal" role="dialog" aria-modal="true">
    ...
    <form className="modal-body" onSubmit={submit}>
      <div className="fields-2">
        ...
        {type === "correction" ? <><label className="field field-wide"><span>Cómo se corrige</span><textarea name="correction_guidance" rows={3} defaultValue={initial?.correction_guidance ?? ""} placeholder="Indicaciones concretas para corregir el error" /></label><label className="field"><span>Medir por</span><select name="measurement_mode" defaultValue={initial?.measurement_mode ?? "both"}><option value="both">Frecuencia + influencia</option><option value="frequency">Frecuencia</option><option value="importance">Influencia</option><option value="none">Sin medición</option></select></label></> : null}
...
function TeachingAssignModal({ student, contents, assignments, terms, close, saved }: { student: Person; contents: TeachingContent[]; assignments: ContentAssignment[]; terms: CatalogTerm[]; close: () => void; saved: () => Promise<void> }) {
  ...
  return <div className="backdrop" onMouseDown={(event) => event.target === event.currentTarget && close()}><section className="modal" role="dialog" aria-modal="true"><header className="modal-head"><div><p className="eyebrow">Enseñar</p><h2>{staffPrimaryName(student)}</h2></div><button className="icon-btn" onClick={close} aria-label="Cerrar"><X /></button></header><div className="modal-body">
...
function TeachingView({ contents, relations, assignments, students, terms, targetContentId, refresh, notify }: { contents: TeachingContent[]; relations: TeachingRelation[]; assignments: ContentAssignment[]; students: Person[]; terms: CatalogTerm[]; targetContentId?: number | null; refresh: () => Promise<void>; notify: (message: string) => void }) {
  ...
  const studentMatches = students.filter((student) => staffPersonMatches(student, studentQuery));
  ...
  const renderContent = (content: TeachingContent) => {
    ...
    return <TeachingContentCard
      key={content.id}
      highlightKey={`teaching-content-${content.id}`}
  ...
  return <>
    <Header eyebrow="Enseñanza" title="Tu biblioteca" description="Crea una vez, relaciona bien y reutiliza en cada clase." action={<button className="btn" data-mission-highlight="create-correction" onClick={() => setCreating(true)}><Plus size={18} /> Crear contenido</button>} />
...
function ScheduleClass({ students, styles, initialStudentId, close, saved }: { students: Person[]; styles: CatalogTerm[]; initialStudentId?: number | null; close: () => void; saved: () => Promise<void> }) {
  ...
  return <div className="backdrop" onMouseDown={(e) => e.target === e.currentTarget && close()}><section className="modal" role="dialog" aria-modal="true">
    ...
    <form className="modal-body" onSubmit={submit}>
      ...
      <div className="fields-2">
        <label className="field"><span>Alumno *</span><select name="student_1" required defaultValue={initialStudentId ? String(initialStudentId) : ""}><option value="" disabled>Seleccionar</option>{students.map((s) => <option key={s.id} value={s.id}>{staffPrimaryName(s)}</option>)}</select></label>
        {type === "pair" ? <label className="field"><span>Segundo alumno *</span><select name="student_2" required defaultValue=""><option value="" disabled>Seleccionar</option>{students.map((s) => <option key={s.id} value={s.id}>{staffPrimaryName(s)}</option>)}</select></label> : null}
...
function StudentPortal({ identity, experience, onExperience, client, email, onIdentityPatch, onOpenProfile, onOpenPreferences }: { identity: IdentityContext; experience: ExperienceContext; onExperience: (value: ExperienceContext) => void | Promise<void>; client: SupabaseClient; email: string; onIdentityPatch: (patch: Partial<IdentityContext>) => void; onOpenProfile: () => void; onOpenPreferences: () => void }) {
  ...
  return <div className="student-portal-shell"><header className="student-portal-head"><Brand /><div><span>{identity.profile_name || snapshot.profile.display_name || identity.display_name}</span><AccountMenu client={client} identity={identity} experience={experience} email={email} variant="header" onExperience={onExperience} onOpenProfile={onOpenProfile} onOpenPreferences={onOpenPreferences} onIdentityPatch={onIdentityPatch} notify={() => undefined} /></div></header><main className="student-portal-main">
    <section className="portal-hero"><div><p className="eyebrow">Mi espacio</p><h1>Hola, {snapshot.profile.first_name || snapshot.profile.display_name}</h1><p>{nextClass ? `Tu próxima clase es ${dateLabel(nextClass.scheduled_start_at)}.` : "Aquí tienes tus clases, saldo y evolución al día."}</p><span className="p0f-teacher-identity">Profesor · CARLOS Y ANDY</span></div><Sparkles /></section>
    ...
    <section className="portal-grid"><article className="card portal-card"><div className="card-head"><h2>Mi Formación</h2><span>{snapshot.assignments.length}</span></div>{snapshot.assignments.length ? <div className="portal-learning-list">{snapshot.assignments.map((assignment) => <TeachingContentCard
        ...
        metadata={[
          ...(assignment.current_frequency !== null ? [{ label: "Frecuencia", value: frequencyLevelLabel(assignment.current_frequency) }] : []),
          ...(assignment.current_importance !== null ? [{ label: "Influencia", value: influenceLevelLabel(assignment.current_importance) }] : []),
        ]}
...
function StaffApp({ session }: { session: Session }) {
  ...
  const [personDetailInitialTab,setPersonDetailInitialTab] = useState<StudentDetailTab>("summary"), [missionHighlight,setMissionHighlight] = useState(""), [missionContentId,setMissionContentId] = useState<number | null>(null);
  ...
  const [unreadNotificationCount, setUnreadNotificationCount] = useState(0), [activeMissionCount, setActiveMissionCount] = useState(0);
  ...
  const loadMissionCount = useCallback(async () => {
    if (!db) return;
    const result = await db.from("missions").select("id", { count: "exact", head: true }).in("state", ["available", "not_done", "postponed", "in_progress"]);
    if (!result.error) setActiveMissionCount(result.count ?? 0);
  }, []);
  const loadStudents = useCallback(async () => {
    if (!db) return;
    const result = await db.from("people").select("id,auth_user_id,display_name,internal_alias,first_name,last_name,email,phone,country_code,crm_stage,active,student_profiles!inner(person_id,active)").eq("active", true).eq("student_profiles.active", true).order("display_name");
  ...
  const loadMarketing = useCallback(async () => {
    ...
    const [contactResult, rateResult, contentResult, eventResult, campaignResult, metricResult, recipientResult] = await Promise.all([
      ...
      db.from("communication_recipients").select("id,campaign_id,person_id,channel,destination,message_snapshot,media_snapshot,status,blocked_reason,prepared_at,sent_at,updated_at,person:people(display_name,internal_alias,country_code),campaign:marketing_campaigns(title)").order("updated_at",{ascending:false}),
    ]);
  ...
  const refreshAllPortalData = useCallback(async () => {
    await Promise.all([loadStudents(), loadOperations(), loadTeaching(), loadMarketing(), loadNotificationCount(), loadMissionCount()]);
  }, [loadStudents, loadOperations, loadTeaching, loadMarketing, loadNotificationCount, loadMissionCount]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    async function boot() {
      ...
      if (nextIdentity?.can_teach || nextIdentity?.can_admin) {
        try { await Promise.all([loadStudents(), loadOperations(), loadTeaching(), loadMarketing(), loadNotificationCount(), loadMissionCount()]); } catch (e) { if (alive) setToast(e instanceof Error ? e.message : "No se pudieron cargar los datos."); }
      }
      if (alive) setReady(true);
    } boot(); return () => { alive = false; };
  }, [session.user.id, loadStudents, loadOperations, loadTeaching, loadMarketing, loadNotificationCount, loadMissionCount]);
  useEffect(() => { if (!toast) return; const timer = window.setTimeout(() => setToast(""), 3000); return () => clearTimeout(timer); }, [toast]);
  useEffect(() => {
    if (!missionHighlight) return;
    let timer = 0;
    let attempts = 0;
    const reveal = () => {
      const target = document.querySelector<HTMLElement>(`[data-mission-highlight="${CSS.escape(missionHighlight)}"]`);
      if (!target && attempts < 24) {
        attempts += 1;
        timer = window.setTimeout(reveal, 100);
        return;
      }
      if (!target) { setToast("El destino existe, pero la acción concreta no está disponible en este estado."); setMissionHighlight(""); return; }
      target.scrollIntoView({ block: "center", behavior: "smooth" });
      target.classList.add("mission-target-highlight");
      if (target.matches("button, input, select, textarea, [tabindex]")) target.focus({ preventScroll: true });
      timer = window.setTimeout(() => { target.classList.remove("mission-target-highlight"); setMissionHighlight(""); }, 2600);
    };
    timer = window.setTimeout(reveal, 80);
    return () => window.clearTimeout(timer);
  }, [missionHighlight]);
  useEffect(() => {
  ...
  function openStudentDetail(student: Person) {
    ...
    setPersonDetailInitialTab("summary");
    setSelected(student);
  }
  ...
  function goTarget(target: string) {
    ...
    if (["home", "students", "classes", "credits", "agenda", "teaching", "marketing", "statistics", "academy", "notifications", "missions"].includes(target)) navigateView(target as View);
  }
  ...
  async function openMissionTarget(snapshotMission: Mission) {
    if (!db) return;
    let mission = snapshotMission;
    if (!mission.source_domain || !mission.source_id || !mission.origin) {
      const result = await db.from("missions").select("id,rule_key,mission_type,state,priority,priority_score,title,description,action_target,source_domain,source_id,origin,due_at,estimated_duration_minutes,calendar_block").eq("id", mission.id).maybeSingle();
      if (!result.error && result.data) mission = { ...mission, ...(result.data as unknown as Mission) };
    }
    const destination = missionDestination(mission);
    if (destination.classId) {
      navigateView("live", { liveClassId: destination.classId });
      setMissionHighlight(destination.highlight);
      return;
    }
    if (destination.personId) {
      const student = students.find((item) => item.id === destination.personId);
      if (!student) { setToast("La Persona de esta misión no está disponible en Alumnado."); return; }
      const state = historyState("students", { selectedId: student.id });
      window.history.pushState(state, "", window.location.href);
      clearTransient();
      setLiveClassId(null);
      setView("students");
      setPersonDetailInitialTab(destination.personTab ?? "summary");
      setSelected(student);
      setMissionHighlight(destination.highlight);
      return;
    }
    const missionViews = new Set(["home", "students", "classes", "credits", "agenda", "live", "teaching", "marketing", "statistics", "academy", "notifications", "missions", "admin"]);
    if (!missionViews.has(destination.view) || (destination.view === "admin" && !activeIdentity.can_admin)) {
      setToast("Esta misión no tiene un destino autorizado en la experiencia actual.");
      return;
    }
    goTarget(destination.view);
    if (destination.contentId) setMissionContentId(destination.contentId);
    setMissionHighlight(destination.highlight);
    if (!destination.exact) setToast("Misión abierta en el mejor destino disponible; falta contexto canónico para señalar una acción más precisa.");
  }
  const studentArea = ["students", "classes", "credits"].includes(view);
  const activeNav = (id: string) => id === "students" ? studentArea : view === id;
  const activeModule = view === "admin" ? "admin" : studentArea ? "students" : view;
  return <div className="shell" data-view={view} data-module={activeModule}>
    <aside className="sidebar"><Brand /><DesktopPrimaryNavigation view={view} studentArea={studentArea} notificationCount={unreadNotificationCount} missionCount={activeMissionCount} navigate={(target) => navigateView(target as View)} />
    ...
    <div><header className="mobile-head"><div className="mobile-head-back">{view !== "home" ? <button className="mobile-back" type="button" onClick={() => goBack("home")} aria-label="Volver">‹</button> : null}</div><div className="mobile-head-brand"><Brand /><span className="mobile-owner-name">Carlos &amp; Andy</span></div><div className="mobile-head-actions"><button className={`icon-btn notification-trigger ${unreadNotificationCount ? "has-notifications" : ""}`} onClick={() => navigateView("notifications")} aria-label={unreadNotificationCount ? `${unreadNotificationCount} notificaciones pendientes` : "Notificaciones"}>{unreadNotificationCount ? <BellRing /> : <Bell />}{unreadNotificationCount ? <span className="notification-dot" aria-hidden="true" /> : null}</button><AccountMenu client={db!} identity={identity} experience={experience} email={accountEmail} variant="header" onExperience={setExperience} onOpenProfile={() => navigateView("profile")} onOpenPreferences={() => navigateView("preferences")} notify={setToast} /></div></header>
      <main className="main"><div className="content">
        ...
        {view === "home" && db ? <HomeView client={db} identity={identity} studentCount={students.length} classes={classes} students={students} go={goTarget} goLive={goLive} openMission={(mission) => void openMissionTarget(mission)} missionsChanged={loadMissionCount} addStudent={openNewStudent} scheduleClass={() => openSchedule(null)} notify={setToast} /> : null}
        ...
        {view === "missions" && db ? <StaffMissionsView client={db} timezone={identity.timezone} openTarget={(mission) => void openMissionTarget(mission)} onPendingChange={setActiveMissionCount} notify={setToast} /> : null}
        ...
        {view === "live" ? <><FeedbackOnlineStaffQueue client={db!} students={students} visible={liveClassId === null} notify={setToast} /><LiveClassView classes={classes} students={students} credits={credits} terms={catalog} library={teachingContents} relations={teachingRelations} assignments={teachingAssignments} selectedClassId={liveClassId} targetHighlight={missionHighlight} selectClass={setLiveClassId} refresh={refreshLive} notify={setToast} exit={() => goBack("home")} /></> : null}
        {view === "teaching" ? <TeachingView contents={teachingContents} relations={teachingRelations} assignments={teachingAssignments} students={students} terms={catalog} targetContentId={missionContentId} refresh={loadTeaching} notify={setToast} /> : null}
        ...
        {view === "marketing" && db ? <MarketingView db={db} contacts={crmContacts.map((contact) => ({ ...contact, canonical_reserved: classes.some((item) => item.status !== "cancelled" && item.class_participants.some((participant) => participant.person_id === contact.id)) }))} rates={marketingRates} content={marketingContent} events={marketingEvents} campaigns={marketingCampaigns} metrics={campaignMetrics} recipients={communicationRecipients} refresh={refreshMarketing} notify={setToast} /> : null}
      </div></main>
      {!isLiveClassSessionActive ? <>
        ...
        {classQuickMenuOpen ? <div className="mobile-class-sheet" role="menu" aria-label="Opciones de clase">
          ...
          <button type="button" role="menuitem" onClick={() => { setClassQuickMenuOpen(false); navigateView("statistics"); }}><BarChart3 /><span><strong>Estadísticas</strong><small>Consultar evolución y actividad</small></span><ChevronRight /></button>
          <button type="button" role="menuitem" onClick={() => { setClassQuickMenuOpen(false); navigateView("marketing"); }}><Megaphone /><span><strong>Marketing</strong><small>CRM, campañas y eventos</small></span><ChevronRight /></button>
          <button type="button" role="menuitem" onClick={() => { setClassQuickMenuOpen(false); navigateView("notifications"); }}><Bell /><span><strong>Notificaciones</strong><small>Novedades de tu portal</small></span>{unreadNotificationCount ? <b className="navigation-count">{unreadNotificationCount > 99 ? "99+" : unreadNotificationCount}</b> : <ChevronRight />}</button>
          <button type="button" role="menuitem" onClick={() => { setClassQuickMenuOpen(false); navigateView("missions"); }}><Target /><span><strong>Misiones</strong><small>Acciones pendientes</small></span>{activeMissionCount ? <b className="navigation-count">{activeMissionCount > 99 ? "99+" : activeMissionCount}</b> : <ChevronRight />}</button>
        </div> : null}
      </> : null}
    </div>
    ...
    {selected && db ? <StudentMasterDetail
      ...
      refresh={async () => {
        if (!db) return;
        await Promise.all([loadStudents(),loadMarketing(),loadTeaching()]);
        const result = await db.from("people").select("id,auth_user_id,display_name,internal_alias,first_name,last_name,email,phone,country_code,crm_stage,active").eq("id", selected.id).maybeSingle();
        if (!result.error && result.data) setSelected(result.data as Person);
      }}
      close={() => goBack(view)}
      schedule={() => openSchedule(selected.id)}
      addCredit={() => openCredit(selected.id)}
      openClass={(id) => goLive(id)}
      initialTab={personDetailInitialTab}
