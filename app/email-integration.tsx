...
export function EmailIntegration({ client, notify }: Props) {
  ...
  async function sendTest(event: FormEvent<HTMLFormElement>) {
    ...
    setSending(true);
    try {
      const response = await authFetch("/api/email/test", { method: "POST", body: JSON.stringify({ to }) });
  ...
  return <article className="card pad">
    ...
    {verified ? <form className="admin-read-list" onSubmit={sendTest}>
      <label className="field">
        ...
        <input type="email" value={testAddress} onChange={(event) => setTestAddress(event.target.value)} placeholder="tu-correo@ejemplo.com" autoComplete="email" required />
      </label>
