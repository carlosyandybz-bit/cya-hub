...
export function P0fEvaluationAdmin({client,terms,notify}:Props){
  ...
  const load=useCallback(async()=>{
    const [milestoneResult,contentResult,pointsResult]=await Promise.all([
      client.from("evaluation_milestones").select("id,style_term_id,role_term_id,level_term_id,aptitude_term_id,milestone_key,label,threshold_score,sort_order,active").order("threshold_score").order("sort_order"),
  ...
  async function removeMilestone(id:number){
    ...
    if(result.error)notify(result.error.message);else{if(editingMilestoneId===id)cancelMilestoneEdit();await load();notify("Hito eliminado.");}setBusy("");
  }
