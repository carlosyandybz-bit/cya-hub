    ...
    {open ? <div className={styles.detailBackdrop} onMouseDown={(event) => event.target === event.currentTarget && setOpen(false)}>
      <section className={styles.detailModal} role="dialog" aria-modal="true" aria-label={`${kindLabel}: ${title}`}>
        ...
        <div className={styles.detailBody}>
          {primaryDetailMedia ? <div className={styles.detailHeroMedia}><SecureDriveAsset
            fileId={primaryDetailMedia.external_file_id}
            mediaType={primaryDetailMedia.media_type}
            title={primaryDetailMedia.title || title}
            thumbnailFileId={primaryDetailMedia.thumbnail_external_file_id}
            controls={primaryDetailMedia.media_type === "video"}
