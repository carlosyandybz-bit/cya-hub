...
type Person={id:number;auth_user_id:string|null;display_name:string;internal_alias:string|null;active:boolean};
...
export function AdminStatistics({client,notify}:{client:SupabaseClient;notify:(message:string)=>void}){
  ...
  const load=useCallback(async()=>{
    const [d,c,p,r,pe,t,mc,a,s,ms]=await Promise.all([
      ...
      client.from("people").select("id,auth_user_id,display_name,internal_alias,active").eq("active",true).order("display_name"),
  ...
  const teacherName=(teacher:Profile)=>{const person=people.find((candidate)=>candidate.auth_user_id===teacher.id);return person?staffPrimaryName(person):teacher.display_name;};
  ...
  function filterEditor(){
    ...
    return <div className="form-row statistics-filter-row">
      {has("teacher")?<label className="field"><span>Profesor</span><select value={filterValues.teacher??""} onChange={(e)=>setFilter("teacher",e.target.value)}><option value="">Todos</option>{teachers.map((teacher)=><option key={teacher.id} value={teacher.id}>{teacherName(teacher)}</option>)}</select></label>:null}
      {has("student")?<label className="field"><span>Alumno</span><select value={filterValues.student??""} onChange={(e)=>setFilter("student",e.target.value)}><option value="">Todos</option>{people.map((person)=><option key={person.id} value={person.id}>{staffPrimaryName(person)}</option>)}</select></label>:null}
