...
import { ArrowDown, ArrowUp, BarChart3, Copy, EyeOff, Plus, Save, Star, UsersRound } from "lucide-react";
...
export function AdminStatistics({client,notify}:{client:SupabaseClient;notify:(message:string)=>void}){
  ...
  async function moveCard(id:number,direction:-1|1){
    const index=selectedCards.findIndex((card)=>card.id===id),target=index+direction;
    if(index<0||target<0||target>=selectedCards.length)return;
    const reordered=[...selectedCards];[reordered[index],reordered[target]]=[reordered[target],reordered[index]];
    setBusy(true);
    ...
    if(error)notify(error.message);else{await load();notify("Orden de tarjetas actualizado.");}
    setBusy(false);
  }
