...
function itemIcon(type: CalendarType) {
  if (type === "mission") return Target;
  if (type === "event") return Megaphone;
  if (type === "external") return RefreshCw;
  return CalendarDays;
}
...
export function AgendaView({ client, timezone, schedule, openClass, notify }: AgendaViewProps) {
  ...
  function renderItem(item: CalendarItem) {
    ...
    return <button className={`calendar-item type-${item.type}`} key={`${item.type}-${item.id}`} onClick={() => item.type === "class" && openClass(item.id)}>
      <span className="calendar-item-icon"><Icon /></span>
      ...
      {item.type === "class" ? <ChevronRight /> : <CircleCheck />}
    </button>;
  }
  ...
  return <>
    ...
    {mode === "month" ? <section className="month-calendar card">
      ...
      {!items.length ? <div className="empty"><CalendarDays /><strong>{loading ? "Actualizando agenda…" : "No hay nada en este periodo"}</strong><p>Los filtros activos no contienen clases, misiones ni eventos.</p></div> : null}
    </section>}
  </>;
}
