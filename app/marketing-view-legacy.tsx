...
function CampaignEditor({ db, item, events, close, saved }: { db: SupabaseClient; item: MarketingCampaign | null; events: MarketingEvent[]; close: () => void; saved: (message: string) => Promise<void> }) {
  ...
  async function submit(event:FormEvent<HTMLFormElement>){
    ...
    const result=await db.rpc("save_marketing_campaign_with_media",{p_campaign_id:item?.id??null,p_title:String(f.get("title")||"").trim(),p_channel:String(f.get("channel")||"whatsapp"),p_objective:String(f.get("objective")||"").trim()||null,p_audience_scope:String(f.get("audience_scope")||"potential"),p_status:String(f.get("status")||"draft"),p_message:String(f.get("message")||"").trim()||null,p_event_id:Number(f.get("event_id")||0)||null,p_budget_cents:f.get("budget")===""?null:Math.round(decimalFormNumber(f.get("budget"))*100),p_scheduled_at:scheduled?new Date(scheduled).toISOString():null,p_starts_at:null,p_ends_at:null,p_media:newDriveMedia(f)});
    if(result.error){setError(result.error.message);setBusy(false);return;}
  ...
  return <div className="backdrop" onMouseDown={(e)=>e.target===e.currentTarget&&close()}><section className="modal campaign-modal" role="dialog" aria-modal="true"><header className="modal-head"><div><p className="eyebrow">Campañas</p><h2>{item?"Editar campaña":"Nueva campaña"}</h2></div><button className="icon-btn" onClick={close} aria-label="Cerrar"><X/></button></header><form className="modal-body" onSubmit={submit}><div className="fields-2">
    ...
    <label className="field"><span>Presupuesto (€)</span><input name="budget" type="text" inputMode="decimal" pattern="[0-9]*([,.][0-9]{0,2})?" defaultValue={item?.budget_cents!=null?item.budget_cents/100:""}/></label>
...
function MetricsEditor({ db, campaign, metrics, close, saved }: { db: SupabaseClient; campaign: MarketingCampaign; metrics: CampaignMetric[]; close: () => void; saved: (message: string) => Promise<void> }) {
  ...
  async function submit(event:FormEvent<HTMLFormElement>){event.preventDefault();const f=new FormData(event.currentTarget);setBusy(true);const result=await db.rpc("save_marketing_campaign_metrics",{p_campaign_id:campaign.id,p_metric_date:String(f.get("metric_date")||today),p_spend_cents:Math.round(decimalFormNumber(f.get("spend"))*100),p_impressions:Number(f.get("impressions")||0),p_reach:Number(f.get("reach")||0),p_clicks:Number(f.get("clicks")||0),p_inquiries:Number(f.get("inquiries")||0),p_bookings:Number(f.get("bookings")||0),p_revenue_cents:Math.round(decimalFormNumber(f.get("revenue"))*100)});if(result.error){setError(result.error.message);setBusy(false);return;}await saved("Resultados guardados.");setBusy(false);close();}
  return <div className="backdrop" onMouseDown={(e)=>e.target===e.currentTarget&&close()}><section className="modal" role="dialog" aria-modal="true"><header className="modal-head"><div><p className="eyebrow">Resultados</p><h2>{campaign.title}</h2></div><button className="icon-btn" onClick={close}><X/></button></header><form className="modal-body" onSubmit={submit}><div className="fields-2"><label className="field"><span>Fecha</span><input name="metric_date" type="date" defaultValue={today}/></label><label className="field"><span>Gasto (€)</span><input name="spend" type="text" inputMode="decimal" pattern="[0-9]*([,.][0-9]{0,2})?" defaultValue={current ? current.spend_cents / 100 : ""}/></label><label className="field"><span>Impresiones</span><input name="impressions" type="text" inputMode="numeric" pattern="[0-9]*" defaultValue={current ? current.impressions : ""}/></label><label className="field"><span>Alcance</span><input name="reach" type="text" inputMode="numeric" pattern="[0-9]*" defaultValue={current ? current.reach : ""}/></label><label className="field"><span>Clics</span><input name="clicks" type="text" inputMode="numeric" pattern="[0-9]*" defaultValue={current ? current.clicks : ""}/></label><label className="field"><span>Consultas</span><input name="inquiries" type="text" inputMode="numeric" pattern="[0-9]*" defaultValue={current ? current.inquiries : ""}/></label><label className="field"><span>Reservas</span><input name="bookings" type="text" inputMode="numeric" pattern="[0-9]*" defaultValue={current ? current.bookings : ""}/></label><label className="field"><span>Ingresos (€)</span><input name="revenue" type="text" inputMode="decimal" pattern="[0-9]*([,.][0-9]{0,2})?" defaultValue={current ? current.revenue_cents / 100 : ""}/></label></div>{error?<p className="error">{error}</p>:null}<div className="actions"><button type="button" className="btn ghost" onClick={close}>Cancelar</button><button className="btn" disabled={busy}>{busy?"Guardando…":"Guardar resultados"}</button></div></form></section></div>;
}
