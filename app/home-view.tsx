...
function personNames(item: ClassSummary, students: PersonSummary[]) {
  return item.class_participants.map((participant) => { const student = students.find((candidate) => candidate.id === participant.person_id); return student ? staffPrimaryName(student) : "Alumno"; }).join(" + ");
}
...
function MissionDialog({ mission, client, close, changed, openTarget }: {
  mission: Mission; client: SupabaseClient; close: () => void; changed: () => Promise<void>; openTarget: () => void;
}) {
  ...
  async function act(action: string) {
    ...
    const result = await client.rpc("act_on_mission", { p_mission_id: mission.id, p_action: action, p_comment: null, p_postpone_until: action === "postpone" ? new Date(postponeUntil).toISOString() : null });
    if (result.error) { setError(result.error.message); setBusy(""); return; }
    await changed(); setBusy(""); close();
  }
  ...
  return <div className="backdrop" onMouseDown={(event) => event.target === event.currentTarget && close()}>
    <section className="modal mission-dialog" role="dialog" aria-modal="true" aria-labelledby="mission-title">
      <header className="modal-head"><div><p className="eyebrow">Misión {missionTypeLabel(mission.mission_type).toLocaleLowerCase("es")}</p><h2 id="mission-title">{mission.title}</h2></div><button className="icon-btn" onClick={close} aria-label="Cerrar"><X /></button></header>
      <div className="modal-body">
        ...
        <button className="btn mission-open-target" type="button" onClick={openTarget}>Ir a la misión <ChevronRight /></button>
        {showPostpone ? <section className="mission-dialog-section postpone-row"><label className="field"><span>Posponer hasta</span><input type="datetime-local" value={postponeUntil} onChange={(event) => setPostponeUntil(event.target.value)} /></label><button className="btn ghost" disabled={Boolean(busy)} onClick={() => act("postpone")}>Confirmar</button></section> : null}
        {error ? <p className="error">{error}</p> : null}
        <div className="actions"><button className="btn ghost" disabled={Boolean(busy)} onClick={() => setShowPostpone((value) => !value)}>Posponer</button><button className="btn" disabled={Boolean(busy)} onClick={() => act("complete")}><CheckCircle2 /> Completar</button></div>
      </div>
    </section>
  </div>;
}

export function HomeView({ client, identity, studentCount, classes, students, go, goLive, openMission, missionsChanged, addStudent, scheduleClass, notify }: HomeViewProps) {
  ...
  return <>
    ...
    {selectedMission ? <MissionDialog mission={selectedMission} client={client} close={() => setSelectedMission(null)} changed={async () => { await Promise.all([load(), missionsChanged()]); }} openTarget={() => { openMission(selectedMission); setSelectedMission(null); }} /> : null}
  </>;
}
