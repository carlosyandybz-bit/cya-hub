...
function cleanColor(value: unknown) {
  return typeof value === "string" && /^#[0-9a-f]{6}$/i.test(value) ? value : "#039BE5";
}
...
export function CalendarVisualAdmin({ client, notify }: Props) {
  ...
  const calendars = useMemo(() => {
    ...
    for (const event of events) {
      ...
      const name = typeof metadata.calendar_name === "string" ? metadata.calendar_name : "Google Calendar";
      const color = cleanColor(metadata.calendar_color);
  ...
  return <article className="card pad calendar-visual-admin">
    ...
    <p>Cada calendario conserva su color de Google. Puedes sustituirlo por uno de la misma paleta y subir una imagen que se usará como icono de todos sus eventos dentro de CYA Hub.</p>
    {!calendars.length ? <div className="compact-empty"><CalendarDays /><span>Sincroniza Google Calendar para cargar sus calendarios y personalizarlos.</span></div> : <div className="calendar-visual-list">{calendars.map((source) => {
      ...
      return <section className="calendar-visual-row" key={source.id} style={{ "--calendar-color": color } as React.CSSProperties}>
        <div className="calendar-visual-preview">
        ...
        <div className="calendar-visual-meta"><strong>{source.name}</strong><small>{source.id === "primary" ? "Calendario principal" : "Google Calendar"}</small>
          <div className="calendar-color-palette" aria-label={`Color de ${source.name}`}>{GOOGLE_PALETTE.map((swatch) => <button type="button" key={swatch} aria-label={`Usar color ${swatch}`} className={(visual?.color_override || source.color).toUpperCase() === swatch ? "active" : ""} style={{ backgroundColor: swatch }} disabled={busy === source.id} onClick={() => void save(source, { color_override: swatch })} />)}</div>
        </div>
