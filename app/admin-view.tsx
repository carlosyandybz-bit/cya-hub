...
export function AdminView({ client, identity, terms, notify, leave }: { client: SupabaseClient; identity: IdentityContext; terms: Term[]; notify: (message: string) => void; leave: () => void }) {
  ...
  function missionsSection() {
    return <section className="admin-stack"><article className="card pad mission-engine-settings"><div className="card-head"><div><p className="eyebrow">Motor</p><h2>Misiones automáticas</h2></div>{data.engine ? <Switch checked={data.engine.enabled} label="Activar motor de misiones" onChange={(checked) => updateRow("mission_engine_settings", "singleton", true, { enabled: checked }, "mission-engine")} /> : null}</div>{data.engine ? <div className="fields-3"><label className="field"><span>Máximo diario</span><input type="number" min="1" max="50" defaultValue={data.engine.max_daily} onBlur={(event) => updateRow("mission_engine_settings", "singleton", true, { max_daily: Number(event.target.value) }, "engine-max")} /></label><label className="field"><span>Revisión diaria</span><input type="time" defaultValue={String(data.engine.daily_review_time).slice(0, 5)} onBlur={(event) => updateRow("mission_engine_settings", "singleton", true, { daily_review_time: event.target.value }, "engine-review")} /></label><label className="field"><span>Horas silenciosas</span><span className="quiet-hours">{String(data.engine.quiet_hours_start).slice(0, 5)}–{String(data.engine.quiet_hours_end).slice(0, 5)}</span></label></div> : null}</article><div className="admin-rule-list">{data.missionRules.map((rule) => <details className="card admin-rule" key={rule.rule_key}><summary><Target /><span><strong>{rule.name}</strong><small>{rule.module_key} · {rule.frequency}</small></span><span className={`badge ${rule.priority === "urgent" ? "mission-urgent" : ""}`}>{rule.priority === "priority" ? "Prioritaria" : rule.priority === "urgent" ? "Urgente" : "Normal"}</span><Switch checked={rule.enabled} label={`Activar ${rule.name}`} onChange={(checked) => updateRow("mission_rules", "rule_key", rule.rule_key, { enabled: checked }, `mission-${rule.rule_key}`)} /><ChevronRight /></summary><div className="admin-rule-body"><p>{rule.description}</p><div className="fields-3"><label className="field"><span>Prioridad</span><select defaultValue={rule.priority} onChange={(event) => updateRow("mission_rules", "rule_key", rule.rule_key, { priority: event.target.value }, `mission-priority-${rule.rule_key}`)}><option value="normal">Normal</option><option value="priority">Prioritaria</option><option value="urgent">Urgente</option></select></label><label className="field"><span>Duración estimada</span><input type="number" min="1" max="480" defaultValue={rule.estimated_duration_minutes} onBlur={(event) => updateRow("mission_rules", "rule_key", rule.rule_key, { estimated_duration_minutes: Number(event.target.value) }, `mission-duration-${rule.rule_key}`)} /></label><label className="field"><span>Máximo diario</span><input type="number" min="1" max="20" defaultValue={rule.max_daily} onBlur={(event) => updateRow("mission_rules", "rule_key", rule.rule_key, { max_daily: Number(event.target.value) }, `mission-max-${rule.rule_key}`)} /></label></div><div className="rule-toggles"><label><Switch checked={rule.auto_complete} label="Completar automáticamente" onChange={(checked) => updateRow("mission_rules", "rule_key", rule.rule_key, { auto_complete: checked }, `mission-auto-${rule.rule_key}`)} /><span>Auto-completar</span></label><label><Switch checked={rule.calendar_block} label="Permitir bloqueo de agenda" onChange={(checked) => updateRow("mission_rules", "rule_key", rule.rule_key, { calendar_block: checked }, `mission-calendar-${rule.rule_key}`)} /><span>Bloque de agenda</span></label></div></div></details>)}</div></section>;
  }

  function notificationsSection() {
    return <section className="admin-stack"><header className="admin-section-head"><div><h2>Avisos y comunicaciones</h2><p>El aviso interno y los envíos externos se configuran por separado.</p></div></header><div className="notification-rule-list">{data.notificationRules.map((rule) => <article className="card notification-rule" key={rule.event_key}><div><Bell /><span><strong>{rule.label}</strong><small>{rule.channels.map((channel) => channel === "internal" ? "Aviso interno" : channel === "email" ? "Email" : channel === "whatsapp" ? "WhatsApp" : channel).join(" · ")}</small></span></div><label className="field"><span>Anticipación</span><input type="number" min="0" max="10080" defaultValue={rule.anticipation_minutes} onBlur={(event) => updateRow("notification_rules", "event_key", rule.event_key, { anticipation_minutes: Number(event.target.value) }, `notification-time-${rule.event_key}`)} /></label><Switch checked={rule.enabled} label={`Activar ${rule.label}`} onChange={(checked) => updateRow("notification_rules", "event_key", rule.event_key, { enabled: checked }, `notification-${rule.event_key}`)} /></article>)}</div></section>;
  }
